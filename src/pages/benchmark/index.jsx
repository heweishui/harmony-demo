    ...
    switch (action.type) {
      case 'RUN':
        return this.setState({ data: buildData(500), selected: 0 },()=>{
          console.log('create500 时长：',`${Date.now()-startTime} ms`)
        })
      case 'RUN_LOTS':
        return this.setState({ data: buildData(1000), selected: 0 },()=>{
          console.log('create1000 时长：',`${Date.now()-startTime} ms`)
        })
      case 'ADD':
        return this.setState({ data: data.concat(buildData(500))},()=>{
          console.log('ADD500 时长：',`${Date.now()-startTime} ms`)
        })
      case 'UPDATE': {
        ...
        return this.setState({ data: newData },()=>{
          console.log('update 时长：',`${Date.now()-startTime} ms`)
        })
      }
      case 'CLEAR':
        return this.setState({ data: [], selected: 0 },()=>{
          console.log('clear 时长：',`${Date.now()-startTime} ms`)
        })
      case 'SWAP_ROWS': {
        if (data.length > 498) {
          return this.setState({ data: [data[0], data[498], ...data.slice(2, 498), data[1], data[499]] },()=>{
            console.log('swap 时长：',`${Date.now()-startTime} ms`)
          })
        }
